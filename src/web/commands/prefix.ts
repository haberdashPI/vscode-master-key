 ...
 * ```
 *
 * These prefixes may be explicitly specified in this way so they can be documented.
 * When users do not provide an explicit prefix, Master key explicitly creates these
 * bindings by itself, but without documentation. As such, all of the bindings written in a
 * `keybinding.json` file have just a single key press, with some conditioned on
 * the specific prefix that must occur beforehand. This is so that master key can
 * explicitly manage state across the key presses of a multi-key sequence keybinding.
 *
 * ## Prefix Format
 *
 * The prefix state is stored under `prefix` (when evaluating an
 * [expression](/expressions/index)) and under `master-key.prefix` in a `when`
 * clause (though it should rarely be necessary to access the prefix explicitly in a `when`
