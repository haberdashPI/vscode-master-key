...
impl Binding {
    ...
    pub(crate) fn new(
        ...
        // NOTE: capture is an automated mode, it shouldn't usually be used,
        // but some special keys (e.g. escape) may want to define a command for it
        let undefined_modes: Vec<_> = mode
            .iter()
            .filter(|x| !scope.modes.contains(x.as_str()) && x.as_str() != "capture")
            .collect();
...
impl ReifiedBinding {
    ...
    pub fn new(binding: &Binding, scope: &mut Scope) -> ReifiedBinding {
        let repeat = match binding.repeat(scope) {
            Ok(x) => x,
            Err(e) => {
                let mut result = ReifiedBinding::noop(scope);
...
impl ReifiedBinding {
    ...
    pub fn resolve_command(&mut self, i: i32, result: &mut KeyFileResult) -> CommandOutput {
        ...
        } else if (i as usize) < self.raw_commands.len() {
            let i = i as usize;
    ...
    pub fn store_command(&mut self, i: usize, result: CommandOutput) {
        if i < self.commands.len() {
            self.commands[i] = result;
        }
    }
}
