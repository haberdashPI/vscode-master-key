...
pub struct DefineInput {
    ...
    pub command: Option<Vec<Spanned<CommandDef>>>,
    pub bind: Option<Vec<Spanned<BindingDef>>>,
}
...
impl VariableExpanding for BindingDef {
    fn expand_with_getter<F>(&mut self, getter: F) -> ResultVec<()>
    where
        F: Fn(&str) -> Result<Option<toml::Value>>,
        F: Clone,
...
impl Define {
    pub fn new(input: DefineInput) -> ResultVec<Define> {
        ...
        let mut resolved_bind = HashMap::<String, Spanned<BindingDef>>::new();
        let mut resolved_command = HashMap::<String, Spanned<CommandDef>>::new();
        ...
        for mut def in input.command.into_iter().flatten() {
            def.get_mut().fields.expand_with_getter(|id| {
                if let Some((prefix, name)) = id.split_once('.') {
                    if prefix == "var" {
            ...
            resolved_command.insert(def.get_ref().id.clone(), def);
        }
        ...
        for mut def in input.bind.into_iter().flatten() {
                ...
                if let Some((prefix, name)) = id.split_once('.') {
                    if prefix == "var" {
                        ...
                        let val = resolved_command
                            .get(name)
                            .ok_or_else(|| Error::UndefinedVariable(format!("`{id}`")))?;
                        return Ok(Some(toml::Value::try_from(val.get_ref())?));
                    } else if prefix == "bind" {
            ...
            resolved_bind.insert(def.get_ref().id.clone(), def);
        }
        ...
        let bind = resolved_bind
            .into_iter()
            .map(|(k, v)| (k, v.into_inner().fields))
            .collect();
        let command = map_with_err(resolved_command, &mut |c| Command::new(c.fields))?;
