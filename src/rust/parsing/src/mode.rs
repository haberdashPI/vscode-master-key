...
impl ReifiedMode {
    pub fn new(mode: &Mode, scope: &mut Scope) -> ResultVec<ReifiedMode> {
        ...
        // as the display name
        let display_name: String = if mode.displayName.is_none() {
            name.clone()
        } else {
            clone_resolve!(mode, displayName, scope)?
        };
    ...
    pub(crate) fn new(
        ...
        if !VersionReq::parse("2.3").unwrap().matches(version) {
            if let Some(name) = &input.displayName {
                ...
                Err(err!("`displayName` is only defined in version 2.3. Bump `version` \
                          to 2.3 or higher.")).with_range(&span)?;
            }
            ...
            if let Some(highlight) = &input.highlight {
                ...
                if let TypedValue::Variable(_) = highlight.as_ref() {
                    Err(err!("`highlight` only supports expressions in version 2.3. Bump \
                            `version` to 2.3 or higher.")).with_range(&span)?;
                }
            }
            if let Some(shape) = &input.cursorShape {
                let span = shape.span().clone();
                if let TypedValue::Variable(_) = shape.as_ref() {
                    Err(err!("`cursorShape` only supports expressions in version 2.3. Bump \
                            `version` to 2.3 or higher.")).with_range(&span)?;
                }
            }
        }
