...
impl TryFrom<toml::Value> for Value {
    ...
    fn try_from(value: toml::Value) -> ResultVec<Self> {
        return Ok(match value {
            ...
            toml::Value::Integer(x) => Value::Number({
...
fn string_to_expression(x: String) -> Value {
    ...
    for expr in exprs {
        ...
        interps.push(TypedValue::Constant(x[last_match.end..r.start].into()));
        last_match = r;

        let var_str = expr.get(1).expect("variable name").as_str();
        interps.push(TypedValue::Variable(Value::Expression(var_str.into())));
    }
    if last_match.start == 0 && last_match.end == 0 {
        return Value::String(x);
    }
    if last_match.end < x.len() {
        interps.push(TypedValue::Constant(x[last_match.end..].into()));
    }
...
pub trait Expanding {
    ...
    where
        Self: Sized + Clone,
    {
        let result = self.clone().map_expressions(&|e| {
...
impl Expanding for Value {
        ...
        F: Fn(String) -> Result<Value>,
    {
        ...
        return Ok(match self {
            ...
            Value::Interp(interps) => Value::Interp(interps.map_expressions(f)?.into()),
            Value::Array(items) => Value::Array(items.map_expressions(f)?),
            Value::Table(kv) => Value::Table(kv.map_expressions(f)?),
            literal @ (Value::Boolean(_) | Value::Number(_) | Value::String(_)) => literal,
        });
    }
}
...
impl From<Value> for toml::Value {
    fn from(value: Value) -> toml::Value {
        return match value {
            ...
            Value::Number(x) => toml::Value::Float(x),
...
impl<T: Serialize + std::fmt::Debug> Expanding for TypedValue<T> {
    ...
    {
        return Ok(match self {
            TypedValue::Variable(v) => TypedValue::Variable(v.map_expressions(f)?),
