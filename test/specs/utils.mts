...
export async function waitForClearedKeyStatus(keySeq: ModalKey[]) {
    const workbench = await browser.getWorkbench();
...
export async function waitForKeysTyped(currentKeySeqString: string) {
    const workbench = await browser.getWorkbench();
    const statusBar = await new StatusBar(workbench.locatorMap);
...
export async function enterModalKeys(...keySeq: ModalKey[]) {
    ...
    waitForClearedKeyStatus(keySeq);
    ...
    for (const keys_ of keySeq) {
        ...
        if (modalKeyUpdateStatus(keys_)) {
            await waitForKeysTyped(currentKeySeqString);
        } else {
            checkCleared = false;
        }
    }
